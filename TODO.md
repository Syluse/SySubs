...
- [ ] **1.1 Graceful worker shutdown on exit** (7.1.1, ties 5.5.4)
...
- [ ] **2.5 Device-switch model caching** (7.1.4)
...
- [ ] **3.1 Centralize error handling** (5.1.1)
...
- [ ] **3.4 Validate config on load** (5.4.6)
...
- [ ] **4.1 Remember last-used folder for Browse dialog** (5.3.5)
  - Pass `initialdir=self.config.get("last_export_folder")` to `askopenfilename`
- [ ] **4.2 Reduce custom-settings input friction** (5.3.6)
  - Switch `trace_add("write")` → `FocusOut` binding; surface validation errors
- [ ] **4.3 Keyboard shortcuts** (5.3.7, 8.4.3)
  - `Ctrl+Enter`/`Ctrl+T` → Transcribe, `Ctrl+S` → Save, `Ctrl+1/2/3` → presets,
    `Ctrl+D` → Reset
- [ ] **4.4 Log panel colorization** (5.3.8, 8.4.4)
  - Pass `LogRecord.levelname` through queue; apply text tags in `_log_to_ui`
- [ ] **4.5 Estimated time remaining** (5.3.3)
...
- [ ] **4.8 Per-segment language logging** (5.7.5)
  - Collect `segment.language`/`language_probability`, log DEBUG per segment + INFO summary
- [ ] **4.9 Model tooltip / multilingual column** (5.7.7)
...
- [ ] **5.1 `resolve("cuda")` must validate CUDA runtime** (8.2.1)
  - Call `_cuda_runtime_available()` inside `resolve()` before returning CUDA DeviceInfo
- [ ] **5.2 Harden `ctypes.CDLL` loading** (7.2.2)
...
- [ ] **6.1 Fix cancel race** (8.1.1)
  - Add `_cancelled` flag; check before processing queued result messages
- [ ] **6.2 Model download timeout** (8.1.2)
  - Wall-clock timeout on download thread; surface friendly error
- [ ] **6.3 Guard against double-click Download** (8.1.3)
  - Disable Download button or track in-flight downloads per model
- [ ] **6.4 Remove dead `PRESETS["multilingual"]`** (8.2.2)
  - Either add to dropdown or remove from constants + handle stale config values
- [ ] **6.5 Fix `_pre_multilingual_lang` restore** (8.2.3)
  - Fall back to `"Auto-detect"` when prior value is missing/invalid
- [ ] **6.6 Initialize `target_save_path` in `__init__`** (8.2.4)
- [ ] **6.7 Don't save geometry when minimized** (8.2.5)
  - Check `window_state() != "iconic"` before saving width/height
- [ ] **6.8 Use `round()` in `_format_srt_time`** (8.2.6)
  - `int(td.microseconds / 1000)` → `round(td.microseconds / 1000)`
- [ ] **6.9 Move SRT write off main thread** (8.4.1)
...
- [ ] **8.1 Parallelize audio extraction + model loading** (5.2.2)
